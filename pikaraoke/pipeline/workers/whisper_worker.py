...
secondary terminal via fd-level dup2.  This captures C-level writes that
Python-level redirects would miss.
...
class WhisperWorker:
    ...
    def _route_to_pty(pty_fd: int | None):
        """Context manager that dup2's the PTY fd over fds 1 and 2.

        Use this to route C-level writes (CTranslate2, stable-ts, etc.)
        to the secondary terminal.
        """
        return _route_to_pty(pty_fd)
    ...
    def unload_model(self) -> None:
        """Unload the model and free GPU memory.  Idempotent — no-op if never loaded."""
        if self._model is not None:
            del self._model
            self._model = None
            self._encoder_module = None
...
def _route_to_pty(pty_fd: int | None):
    ...
    Restores saved originals on exit. No-op when pty_fd is None.

    Used to capture C-level writes (CTranslate2, FFmpeg spawned by
    AudioLoader, etc.) that Python-level redirects would miss.
    """
    if pty_fd is None:
        yield
        return
