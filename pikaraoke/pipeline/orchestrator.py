...
from pikaraoke.pipeline.stages.base import PipelineStage
...
class PipelineOrchestrator:
    ...
    def start(self) -> None:
        ...
        rather than mid-queue on a subsequent song.  ``_workers_started`` is
        set *before* spawning so that if the second worker fails to start
        (the OOM this eager start exists to surface), a follow-up ``stop()``
        still tears down the worker that did start instead of leaking it.
        """
