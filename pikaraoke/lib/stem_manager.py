...
class StemManager:
    ...
    def _process_queue(self) -> None:
        """Worker loop: process songs from the queue serially."""
        while True:
            ...
            self.active_job = {"path": song_path, "status": "starting"}
            ...
            try:
                ...
                self._events.emit(
                    "notification",
                    f"Stem separation failed: {Path(song_path).name}",
                    "danger",
                )
            finally:
    ...
    def _process_song(self, song_path: str) -> None:
        ...
        self.active_job = {"path": song_path, "status": "extracting audio"}
        self._events.emit(
            "notification",
            f"Processing stems: {video.name}",
        )

        tmp_dir = tempfile.mkdtemp(prefix="pikaraoke_stems_")
        try:
            # Step 1: Extract audio to WAV
            audio_wav = self._extract_audio(video, tmp_dir)

            # Step 2: Separate into vocal + instrumental WAV stems
            self.active_job["status"] = "separating stems"
            vocals_wav, instrumental_wav = self._separate_stems(audio_wav, tmp_dir)

            # Step 3: Transcode both stems to M4A
            self.active_job["status"] = "transcoding"
            self._wav_to_m4a(vocals_wav, vocal_out)
            self._wav_to_m4a(instrumental_wav, nonvocal_out)

            logging.info(f"Stem separation complete: {video.name}")
            self._events.emit(
                "notification",
                f"Stems ready: {video.name}",
                "success",
            )
        finally:
            shutil.rmtree(tmp_dir, ignore_errors=True)

    def _extract_audio(self, video_path: Path, tmp_dir: str) -> Path:
    ...
    def _separate_stems(self, audio_path: Path, tmp_dir: str) -> tuple[Path, Path]:
    ...
    def _wav_to_m4a(self, wav_path: Path, output_path: Path) -> None:
