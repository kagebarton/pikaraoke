...
def _interpolate_missing(
    ...
    """Fill in lines that had no selected candidate by linear interpolation
    across their bracketing placed neighbours.

    Output is 1:1 with the lyric line list, sorted by ``line_id``. An
    interpolated line carries ``source: "interp"`` and per-word timings
    equally spaced inside its allocated window. A line with no tokens
    yields an empty-words object with ``source: "absent"``.
    """
