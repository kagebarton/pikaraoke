...
class MpvController:
    ...
    def start(self, audio_device: str | None = None, audio_delay: float = 0.0) -> None:
        ...
        try:
            ...
            log.warning("No audio server found (wpctl/pactl/amixer). Volume controls disabled.")
            self._audio_backend = None

        self.is_running = True
        log.info("MPV started successfully (python-mpv / libmpv)")
    ...
    def quit(self) -> None:
        ...
        if p is not None:
            ...
            try:
                ...
                log.debug("mpv quit/wait_for_shutdown raised during teardown", exc_info=True)
            try:
                p.terminate()
            except Exception:
                log.debug("mpv terminate raised during teardown", exc_info=True)
    ...
    def play(
        ...
        *,
        ...
        p = self._player
        if p is None:  # never started, or torn down on another thread
            return
        ...
        try:
            p.pause = False  # also clear MPV's pause property (pause-on-start fix)
            self.position = 0.0

            # Reset duration alongside the readiness event: if _on_duration never
            # fires for the new file (the wait below times out) self.duration must
            # not retain the previous song's value. play() and the observer are the
            # only writers.
            self.duration = 0.0
            self._duration_ready.clear()
            self._fired_osd_dim = (None, None)  # force overlay refresh on resize
            p.loadfile(file_path)

            if not self._duration_ready.wait(timeout=5):
                log.warning("Duration not ready after 5s -- proceeding anyway")

            # Add audio tracks for dual-stem playback. audio-add is processed
            # asynchronously by the libmpv core, so wait for both tracks to
            # register before building a filter that references [aid2]/[aid3] --
            # a fixed sleep races the core (too short drops the filter inputs).
            if self._dual_stem:
                before = self._audio_track_count()
                p.command("audio-add", vocal_path, "auto")
                p.command("audio-add", nonvocal_path, "auto")
                self._wait_for_audio_tracks(before + 2, timeout=2.0)
            ...
            # Final synchronous read as a fallback; keep the observer's value if the
            # read isn't ready (don't clobber a known-good duration with 0.0).
            synced_duration = float(p.duration or 0.0)
            if synced_duration > 0:
                self.duration = synced_duration
        except mpv.ShutdownError:
            # quit() ran on another thread mid-play; abort cleanly.
            log.debug("play() aborted: mpv shutting down")
            return
        # no set_mode — caller decides when to render

    def _audio_track_count(self) -> int:
        """Number of audio tracks libmpv currently reports (0 if unavailable)."""
        p = self._player
        if p is None:
            return 0
        try:
            return sum(1 for t in (p.track_list or []) if t.get("type") == "audio")
        except Exception:
            return 0

    def _wait_for_audio_tracks(self, target: int, timeout: float) -> None:
        """Block until libmpv reports at least ``target`` audio tracks, or timeout.

        Replaces a fixed post-``audio-add`` sleep: the libmpv core registers
        added tracks asynchronously, so polling the track list is both faster
        on a quick core and safe on a slow one.
        """
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            if self._audio_track_count() >= target:
                return
            time.sleep(0.01)
        log.warning(
            "Dual-stem audio tracks not ready after %.1fs (have %d, want %d)",
            timeout,
            self._audio_track_count(),
            target,
        )
    ...
    def stop(self) -> None:
        ...
        p = self._player
        if p is None:  # already torn down
            return
        ...
        # re-entering on_song_end. self._lock serializes the clear, so a second
        # on_song_end on the event thread would block on it while stop() runs.
        self.is_idle = True
        try:
            with self._lock:
                p.lavfi_complex = ""
            ...
            # quit() ran on another thread; the player is gone, nothing to stop.
            pass
    ...
    def get_system_volume(self) -> int:
        ...
        try:
            if self._audio_backend == "wpctl":
                ...
                # wpctl appends "[MUTED]" when the sink is muted but still prints
                # the underlying (non-zero) volume; report 0 so a muted sink isn't
                # read as audible.
                if "MUTED" in r.stdout:
                    return 0
            ...
            log.warning("Failed to read system volume (%s); assuming 100", e)
        return 100
    ...
    def set_system_volume(self, percent: int) -> None:
        ...
        try:
            ...
            log.warning("Failed to set system volume: %s", e)
