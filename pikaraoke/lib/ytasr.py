"""YouTube auto-caption (ASR) parsing + candidate-span derivation for the joint matcher.

Genius-origin (txt) songs reach the matcher with no per-line cue times. YouTube's
auto-generated captions are a free, *same-clock* timing source, carrying per-word
timestamps in the ``json3`` caption format. This module turns a downloaded
``json3`` track into a third candidate source for the joint DP matcher
(``pikaraoke/lib/joint_match.py``): parse -> quality-gate -> derive candidate
spans.

Two failure modes are gated out before adoption (either drops ytasr, leaving
plain two-source joint matching):
...
Candidate spans are derived per line by fuzzy candidate search, NOT a single
...
def is_usable(words: list[dict], word_seg_frac: float, media_dur: float | None) -> bool:
    ...
    Two gates (either failure -> drop ytasr, run two-source): the word-seg
    fraction must clear :data:`WORD_SEG_MIN_FRAC` (reject manual-mirrored/
    line-level), and the real words-per-minute over ``media_dur`` must clear
    :data:`MIN_CAPTION_WPM` (reject ``[Music]`` degeneracy). An unknown
    ``media_dur`` is rejected: the degeneracy gate can't run without it, so
    adopt nothing rather than risk a sparse caption unchecked (never feed the
    matcher a caption it can't vet).
    """
