...
def _ass_escape(text: str) -> str:
    """Neutralize ASS-events metacharacters in user-supplied text.

    Song titles and singer names may contain '{' or '}' (which delimit ASS
    override blocks) or '\\' (which forms line-break/spacing codes like \\N,
    \\n, \\h). Left raw they would inject styling tags or line breaks into the
    OSD. Braces become parens; backslash becomes a visually similar glyph so
    the text still renders literally.
    """
...
def _build_nowplaying_overlay(state: OverlayState, fs: int) -> Overlay:
    return Overlay(
        ...
        text=f"{_ICON_PLAY} {_ass_escape(state.now_playing_title or '')}",
    )
...
def _build_timecode_overlay(state: OverlayState, fs: int) -> Overlay:
    ...
    text = (
        f"{_ICON_MIC_SINGER} {_ass_escape(state.singer_name)} | "
        f"{elapsed} / {total} | {_ICON_CLEF} {st_str}"
    )
    if state.dual_stem:
...
def _build_upnext_overlay(state: OverlayState, fs: int) -> Overlay:
    ...
    singer_part = f"{{\\c{_COLOR_SINGER_BLUE}}}{_ICON_MIC_SINGER} {_ass_escape(song.singer)}"
    return Overlay(
        id=OSD_UPNEXT,
        anchor="\\an9",
        pos=(1920, y),
        font_size=fs - 10,
        color=_COLOR_UPNEXT,
        text=f"{_ICON_NEXT} {_ass_escape(song.title)} {singer_part}",
    )
...
def _build_queue_preview_overlay(state: OverlayState, fs: int) -> Overlay:
    ...
    singer1 = f"{{\\c{_COLOR_TIMECODE}}}{_ICON_MIC_SINGER} {_ass_escape(songs[0].singer)}"
    first = f"{_ICON_NEXT} {_ass_escape(songs[0].title)} {singer1}"
    # Remaining rows: blue, singer in lighter blue
    rest = [
        f"{i}. {_ass_escape(s.title)} "
        f"{{\\c{_COLOR_SINGER_BLUE}}}{_ICON_MIC_SINGER} {_ass_escape(s.singer)}"
...
class OverlayManager:
    ...
    def apply(self, state: OverlayState) -> None:
        ...
        for oid in ALL_OSD_IDS:
            ...
            elif new is not None and (new != old or self._force_resend):
    ...
    def invalidate(self) -> None:
        """Force the next apply() to re-send every still-desired overlay.

        Keeps _last_sent so the next apply() still clears overlays that are no
        longer desired -- clearing it would orphan them on screen (the diff loop
        would see both old and new as None and skip the clear).
        """
