...
right-text/wrong-sync variant can be told apart at scoring time.
"""
...
def cue_spans_from_lrc(lrc_text: str) -> tuple[list[str], list[tuple[float, float]]]:
    ...
    Parallel to :func:`pikaraoke.lib.srt_cues.cue_spans_from_srt` but for
    LRC: LRCLIB exports stamp only starts, so each line ends at the next
