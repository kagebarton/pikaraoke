"""LRCLIB synced-lyrics fetch + selection — HARNESS-ONLY held-out scoring reference.

Vendored from the former ``pikaraoke/lib/lrclib.py`` (deleted from production
when LRCLIB was dropped as a timing prior — see commit 988725ce). The full-mix
alignment experiments keep LRCLIB purely as an *out-of-band scoring reference*:
a set of held-out per-line cue spans that placements are measured against, never
fed into any matcher. It lives under ``scripts/`` so the production tree carries
no dead LRCLIB code.

The full-mix sweep harnesses (``replay_fullmix_source.py``) read a song's synced
lyrics from the flat per-song cache ``<root>/lrclib/<stem>`` when present, else
fetch fresh via :func:`search` + :func:`select_candidate`, then reduce to
per-line cue spans with :func:`cue_spans_for_lines` and score with
``pikaraoke.lib.srt_cues.offset_mad_against_cues``.

Selection is reference-free: rank candidates by how well their text maps to our
lyric sheet (:func:`map_lines_to_cues`), with the video duration as a tiebreak.
"""
