...
(2026-07-14, full context in that plan): verified on YouTube that none
of the three songs has an uploader SRT — their on-disk
`subtitles/<stem>.srt` files are pipeline-generated, so all three
bundles' `ground_truth_refs.youtube_srt_present: True` is contamination
from the stage's stale-SRT self-adoption on a second run, not a real
caption.

**Backup.** All three original bundle JSONs copied unmodified to
`D:\shared\pikaraoke-songs\provenance_backup_20260714T155515Z\` before
any edit; SHA-256 of each backup verified equal to its source.
(`Copy-Item`'s default `-Path` parameter silently copied zero bytes for
the Bloodstream file on the first attempt — its name contains a literal
`[...]` run that PowerShell's default path parameter treats as a
wildcard glob; `-LiteralPath` fixed it. Caught immediately by a
post-copy directory listing showing 2 files instead of 3, before any
bundle was touched; the SHA-256 check afterward is the recorded
byte-identical confirmation.)

**Precondition check + patch** (`patch_bundles.py`, scratchpad,
two-phase: assert all three, then write all three). Asserted per bundle:
`pipeline_decisions.method_used == "joint"`,
`ground_truth_refs.youtube_srt_present is True`,
`ground_truth_refs.youtube_srt_is_lyric_source is False`. All three
passed on first read (no STOP). Each was then rewritten with
`ground_truth_refs.youtube_srt_present = False` and
`.youtube_srt_path = None`, serialized identically to
`alignment_capture.write_bundle` (`json.dumps(bundle, indent=2,
ensure_ascii=False, default=_json_default)`, UTF-8, atomic
tmp-then-replace).

**Diff verification** (`verify_diff.py`, scratchpad — unified diff,
backup vs patched, zero context): all three show exactly the two
expected changed lines, nothing else:

- `'Defying Gravity' - Wicked 20th Anniversary Edition ... AoON1CyhQAM.json`:
  `youtube_srt_present: true → false`;
  `youtube_srt_path: "subtitles\\'Defying Gravity'...AoON1CyhQAM.srt" → null`.
- `Ed Sheeran & Rudimental­ - Bloodstream [Official Music Video­
  YTMAs]---Orq_75kFi8I.json`: same two fields, same transform.
- `HUNTR_X 'This Is What It Sounds Like' (Music Video) _ KPop Demon
  Hunters _ Netflix Philippines---hI-y5anGcUA.json`: same two fields,
  same transform.

**Generated-SRT markers** (Part 2's convention, written inert ahead of
Part 2's code per Ken's ruling): `mark_generated_srts.py` (scratchpad)
wrote `subtitles/<stem>.srt.generated` next to all three songs' `.srt`
files, one line each: `Generated by PiKaraoke's lyric-align stage; not
an uploader caption.` Each target path was cross-checked two independent
ways (the backup bundle's recorded `youtube_srt_path`, and an
independent substring glob over `subtitles/*.srt`) before writing; all
three agreed, and no pre-existing marker was found at any of the three
paths.

**Harness invocations**, repo root, both `replay_ytasr_third_source.py
"D:\shared\pikaraoke-songs" --alpha 2.0 --beta 2.0`: once before any
bundle edit (14 rows), once after the patch + marker writes above (17
rows).

(Capture-method note: the pre-patch run was captured via a PowerShell
`Tee-Object` pipe; none of its 14 songs' filenames need characters
outside cp1252, so this was lossless. The first post-patch capture via
the same pipe silently mangled the Bloodstream row's non-breaking-space
and soft-hyphen characters to U+FFFD — PowerShell decodes a piped child
process's stdout using the console codepage before `Tee-Object` ever
sees it; caught because the corrupted bytes then failed to re-encode on
a later `print()`, not silently. Re-captured post-patch via a raw Bash
`>` redirect with `PYTHONUTF8=1 PYTHONIOENCODING=utf-8`, which writes
true UTF-8 with no lossy intermediate decode; that clean file is the one
below.)

Pre-patch table (14 rows, `prepatch_harness.txt`):

```
song                                            src        mad(best) alpha  beta crawl(rec>new)   overlap(rec>new)  placed(rec>new)   coverage
----------------------------------------------------------------------------------------------------------------------------------------------
'Free' _ Official Lyric Video _ Sony Animation 2src        0.18s/16a   2.0   n/a      2->1           0.0->1.0          40->40          40/41
...
Josh Gad - In Summer (From 'Frozen'_Sing-Along 2src        0.36s/14a   2.0   n/a      2->2           0.0->0.0          29->29          29/31
Mulan _ I'll Make a Man Out of You _ @disneyki 3src bail:wide_spread   2.0   2.0      0->0           0.0->0.0          36->36         36/47!
NSYNC - Paradise                               2src        0.27s/10a   2.0   n/a      4->3           0.0->0.7          55->55         55/65!
Pocahontas - Colors of the Wind (Blu-ray 1080p 3src        0.17s/31a   2.0   2.0      2->2           0.0->0.0          37->37          37/37
Seasons of Love (HD)---UvyHuse6buY             2src bail:few_anchors   2.0   n/a      8->3           0.0->0.0          26->25         25/34!
The Lion King - Hakuna Matata Music Video I 4K 3src         0.35s/8a   2.0   2.0      3->2           0.0->0.0          33->33         33/40!
The Next Ten Minutes Lyrics---0j8kL24ph8U      2src        0.46s/52a   2.0   n/a      6->2           0.0->0.0          68->67          67/71
Wicked - For Good  (2025) 4K - The Girl in the 3src        0.38s/15a   2.0   2.0      0->1           0.0->0.4          26->29         29/36!
```

Post-patch table (17 rows, `postpatch_harness_utf8.txt`):

```
song                                            src        mad(best) alpha  beta crawl(rec>new)   overlap(rec>new)  placed(rec>new)   coverage
----------------------------------------------------------------------------------------------------------------------------------------------
'Defying Gravity' - Wicked 20th Anniversary Ed 3src bail:wide_spread   2.0   2.0      3->3           2.2->0.2          51->51         51/89!
'Free' _ Official Lyric Video _ Sony Animation 2src        0.18s/16a   2.0   n/a      2->1           0.0->1.0          40->40          40/41
'Popular' - Wicked 20th Anniversary Edition _  3src bail:wide_spread   2.0   2.0      3->3           0.0->0.0          52->52         52/62!
Beauty and the Beast (1991) - Be Our Guest [UH 3src        0.16s/49a   2.0   2.0      1->0           0.0->0.0          77->77          77/77
Beauty and the Beast (1991) - Belle [UHD]---ot 3src        0.39s/55a   2.0   2.0      1->1           0.0->0.0         101->101       101/110
Ed Sheeran - Best Part Of Me (feat. YEBBA) (Li 3src bail:wide_spread   2.0   2.0      2->2           0.0->0.0          37->37          37/38
Ed Sheeran & Rudimental­ - Bloodstream [Offici 3src        0.58s/11a   2.0   2.0      1->1           0.0->6.7          47->48         48/74!
...
**Row comparison** (`compare_harness_tables.py`, scratchpad, tokenized
per column rather than hand-derived fixed-width offsets): of the 17
post-patch rows, 14 keys match a pre-patch row exactly (0 missing), and
exactly 3 are new — the three target songs, matching the plan's step-5
pre-registration. Of the 14 shared rows: **14/14 exact-match** on every
column (song, src, mad, alpha, beta, crawl, overlap, placed, coverage);
0 required the LRCLIB-timeout flake carve-out. No STOP condition fired.

Per the plan's pre-registered consequence (step 5-6), this 17-row table
supersedes the 14-row `replay_post_e3a.txt` values as the post-Phase-4
baseline. Executor stops at these artifacts; per established process
(this doc's Phase 4 GATE close) the judge reads them before the table
above is blessed as the adopted baseline.
...
### Environment note — Linux-box fresh-corpus harness baselines (2026-07-16)

This box (`fable_matcher_refine`) became primary this session: its
33-song `alignment_debug` corpus was regenerated from scratch on local
GPU (a corpus-wide missing-`.generated`-marker bug was found and fixed
mid-regen; full details in this machine's Claude memory, not repeated
here since nothing below depends on it beyond "the corpus is now
current"). Both of Phase 0's step-7 baseline runs were captured fresh
against that corpus — the non-SRT one because the corpus changed
machines, the SRT/cue one for the first time ever (deferred at Phase 0
as "optional now, required before Phase 5", never run on any box until
now).

**Non-SRT baseline** —
`replay_ytasr_third_source.py /home/ken/pikaraoke-songs --alpha 2.0
--beta 2.0`, run 3 times. Verified programmatically (not by eye): **0
songs show any crawl/overlap/placed/coverage mismatch across the 3
runs** — full determinism on every matcher-computed column. The only
run-to-run variance is *which* live-fetch song's LRCLIB lookup times
out (`HTTPSConnectionPool... Read timed out`, a 30s network flake in
the held-out scoring tier this box hits more than the Windows box did);
each of the 3 flake-affected songs (Defying Gravity, Domino, Seasons of
Love) recovered a real MAD/bail value on at least one of the 3 runs.
Composite table (best-of-3 per song, `ytasr_baseline_composite.txt`):
...
HUNTR_X 'This Is What It Sounds Like' (Music V  2src  bail:wide_spread   2.0   n/a            0->0          0.0->0.0           32->33    33/53!
Jessie J - Domino (Official Video)---UJtB55Mao  2src          0.43s/7a   2.0   n/a            0->0          0.0->0.1           63->63     63/67
...
No prior Linux-box table exists to diff this against, and a cross-machine
diff against the Windows `postpatch_harness_utf8.txt` (2026-07-14, this
doc's line ~2503) is expected to show deltas on both `rec` and `new`
sides for most songs — the whole corpus behind it was regenerated by a
different machine's whisper/GPU stack, so the captured `words`/
`transcribe_words` inputs the matcher replays from are themselves not
byte-identical to the Windows capture. This table, not that one, is
this box's reference point for later phases.

**SRT/cue baseline** — `cue_align_corpus.py` (GPU). Smoke-tested on one
song first (`--only Happier`, 38/38 placed, 0 drift, ~2 min) before
committing to the full batch; the full-batch row for that same song
(`plc=38 hid=0 rea=0 rep=0 maxgap=1.4s gapL=0 instL=0 ovl=0.0->0.0`) is
byte-identical to the smoke test, one more small determinism check.
Full run: 16/16 SRT-sourced songs completed (0 skipped, 0 failed),
~25 min wall-clock. Flag table (`cue_align_baseline_full.log`):

```
song                                        plc  hid  rea  rep rsec  maxgap gapL instL   ovl(old>new)
----------------------------------------------------------------------------------------------------
Justin Timberlake - Like I Love You (Offic   84    0    3    1    -    2.1s    0     0   0.0->0.0  s
Backstreet Boys - More Than That---1OwfYje   39    0    3    2    -    1.9s    0     0   0.0->0.0  s
ZAYN, Zhavia Ward - A Whole New World (End   60    0    1    1    -    1.9s    0     0   7.8->7.5  s
Idina Menzel - Let It Go (from Frozen) (Of   47    0    0    0    -    1.9s    0     0   0.0->0.0  s
Naomi Scott - Speechless (from Aladdin) (O   49    0    0    0    -    1.9s    0     0   0.0->0.0  s
Justin Timberlake - Mirrors (Official Vide  120    0   22    7    -    1.8s    0     0   0.7->2.6  s
#OutOfOz - 'For Good' Performed by Kristin   60    0    6    3    -    1.8s    0     0   1.6->1.7  s
Justin Timberlake - Selfish (Official Vide   79    0    0    0    -    1.8s    0     0   0.0->0.0  s
Ariana Grande, John Legend - Beauty and th   56    0    1    4    -    1.7s    0     0   0.2->0.2  s
Justin Timberlake - Rock Your Body (Offici  103    0    0    0    -    1.6s    0     0   0.0->0.0  s
Jodi Benson - Part of Your World (From 'Th   54    0    0    2    -    1.5s    0     0   0.7->0.7  s
The Lion King - Can You Feel The Love Toni   32    0    1    0    -    1.5s    0     0   0.0->0.0  s
Ed Sheeran - Happier (Official Music Video   38    0    0    0    -    1.4s    0     0   0.0->0.0  s
Backstreet Boys - Incomplete (Official HD    27    0    0    0    -    1.3s    0     0   0.0->0.0  s
Mena Massoud, Naomi Scott - A Whole New Wo   54    0    1    0  yes    1.3s    0     0   2.6->2.6  s
NSYNC - Bye Bye Bye (Official Video)---Eo-   75    0    0    1  yes    0.8s    0     0   1.3->1.3  s

prevalence: 0/16 songs show drift; 0 parked-tail line(s), 0 mostly-instant line(s) total
```

`gapL`/`instL` are the two drift-artifact counts the script exists to
measure (`n_gap`: lines with an internal word-gap past the parked-tail
threshold; `n_instant`: lines more than half instant/failed words) —
both are **zero on every one of the 16 songs**. `maxgap` is just the
worst raw internal gap seen per song (informational, not itself a
defect) so its 0.8-2.1s spread is not a signal. `ovl(old>new)` compares
the shipped pipeline's own max inter-line overlap (`old`, from each
bundle's `output_line_timings`) against this cue-align replay's own
(`new`): flat or improved on 15/16 songs, one regression — **Justin
Timberlake "Mirrors" 0.7s → 2.6s**, worth a closer look before/during
Phase 5 (also the song with by far the most realigned/re-paced lines,
22+7 of 120, so it is doing the most corrective work of any song here).

Two of the three whisper narrow-slice warnings already on file from
this session's bundle regen recurred here too, independently — same
song, same exact line(s), but hit by cue-align's own fresh alignment
call rather than by the bundle's: OutOfOz line 50, and NSYNC - Bye Bye
Bye lines 73-74 (both `'NoneType' object has no attribute 'language'`,
both gracefully re-paced from cue, both already reflected in `rep`
above). Two independent code paths hitting the identical song+line is
good evidence this is a real, narrow, content-specific `stable_whisper`
edge case rather than a fluke of either call site.

This satisfies Phase 0's deferred "required before Phase 5" precondition
— cue-path work can now proceed with a real flag-count baseline to diff
against, on this box.
