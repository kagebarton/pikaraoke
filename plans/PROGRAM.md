...
| Karaoke fill per word (ASS writer) | each word fills for at least 0.1 s, and the overshoot carries to the rest of the line | **same — unchanged** (Ken, 2026-09-21; edge snap plan, "Writer 10-cs floor"): it only makes lines late where the aligner gave words no time (lyrics not sung in the audio, a sung echo), so removing it corrects no well-timed line |
...
| `edge-snap-coverage-accuracy.md` | the edge snap post-pass track, **not a routing lane**: onset silence gate, one-word lines, stem-end and end-reference fixes, interior run edges — **Phases 0-5 and 5.1 accepted 2026-09-18**; production code goes to Ken's user-test branch before `master`. Phase 6 NOT PINNED (Ken's call); **Phase 7a CLOSED 2026-09-21 (Ken): the voicing trace isn't worth its processing time, no production change**; 7b-7d gated; **the ASS writer's 10-cs floor (raised by the 7a read-off) CLOSED 2026-09-21 (Ken): it only bites on words the aligner gave no time, no production change** |
